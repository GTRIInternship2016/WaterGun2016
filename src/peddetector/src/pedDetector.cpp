...
//    printf("firstline");
  ...
  // Find our root node
    ...
    widthOverHeight = 0.41;
    shrinkFactor = 4;
    theoWWidth = 52;
    theoWHeight = 128;
    theoActWWidth = 39.36;
    theoActWHeight = 96;
    rectFile = "rectangles.dat";
    nrFeatures = 3000;
    nrProp = 5;
    classFile = "StrongClassifierTreeStep9.txt";
    nrClass = 1000;
    nrCol = 14;
    nBaseFeatures = 3000;
    nExtraFeatures = 10;
...
//    verbose = verboseN->first_attribute("value")->value() != "0";
...
vector<Rect_<int> >* pedDetector(Mat img_original){
  ...
  helperXMLParser parsed = helperXMLParser("~/watergun_2016/src/peddetector/files/configuration.xml");
  if(parsed.verbose)
    parsed.print();
  ...
  delete(pInput->pad);
  int *pad = new int[2];
  pad[0] = sctInput->theoreticalVerticalPadding;
  pad[1] = sctInput->theoreticalHorizontalPadding;
  pInput->pad = pad;
